...
- Set an admin token with the `ADMIN_ACCESS_TOKEN` environment variable (defaults to `letmein` for local use).
- Start the dev server and open `/admin/login`, then enter the same token to issue an admin cookie.
